...
export const checkEmail = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const { email } = req.body;
  await AuthService.check(email);
  res.status(200).json({ message: 'Email is not taken', statusCode: 200 });
});

export const tempUser = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const { email } = req.body;
